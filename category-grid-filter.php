<?php
/**
 * Plugin Name:  Grid Layout Category Filter For Posts
 * Author:       Speedwrks 
...
// Defines
define('spwrk_post_grid_VERSION', '1.0');

/**
* Loading Text Domain
*/
add_action('plugins_loaded', 'spwrk_post_grid_plugin_loaded_action', 10, 2);
function spwrk_post_grid_plugin_loaded_action() {
	load_plugin_textdomain( 'spwrk_post_grid', false, dirname( plugin_basename(__FILE__) ) . '/lang/' );
}
...
// Enqueue scripts
function spwrkafp_scripts(){

	// CSS File
	wp_enqueue_style( 'spwrkafp-styles', plugin_dir_url( __FILE__ ) . 'assets/css/spwrk-grid-styles.css', null, spwrk_post_grid_VERSION );

	// JS File
	wp_register_script( 'spwrk_ajax_filter_post', plugin_dir_url( __FILE__ ) . 'assets/js/spwrk-grid-scripts.js', array('jquery'), spwrk_post_grid_VERSION );
	wp_enqueue_script( 'spwrk_ajax_filter_post' );

	// Localization
	wp_localize_script( 'spwrk_ajax_filter_post', 'spwrk_ajax_params', array(
	        'spwrk_ajax_nonce' => wp_create_nonce( 'spwrk_ajax_nonce' ),
	        'spwrk_ajax_url' => admin_url( 'admin-ajax.php' ),
	    )
	);
}

add_action( 'wp_enqueue_scripts', 'spwrkafp_scripts' );

//shortcode function
function spwrk_post_grid_shortcode_mapper( $atts, $content = null ) {
	...
	<div class="am_ajax_post_grid_wrap" data-pagination_type="<?php echo esc_attr($pagination_type); ?>" data-am_ajax_post_grid='<?php echo json_encode($shortcode_atts);?>'>
		...
		<?php if ( $show_filter == "yes" && $terms && !is_wp_error( $terms ) ){ ?>
			<div class="spwrk-filter-div" data-layout="<?php echo $layout; ?>"><ul>
				<?php if($btn_all != "no"): ?>
					<li class="spwrk_texonomy" data_id="-1"><?php echo esc_html('All','spwrk_post_grid'); ?></li>
				<?php endif; ?>
		        <?php foreach( $terms as $term ) { ?>
		            <li class="spwrk_texonomy" data_id="<?php echo $term->term_id; ?>"><?php echo $term->name; ?></li>
		        <?php } ?>
	        </ul></div>
	    <?php } ?>

	    <div class="spwrk-ajax-container">
		    <div class="spwrk-loader">
		    	<div class="lds-dual-ring"></div>
		    </div>
		    <div class="spwrkafp-filter-result"></div>
	    </div>
    </div>

	<?php return ob_get_clean();
}
add_shortcode('spwrk_ajax','spwrk_post_grid_shortcode_mapper');
add_shortcode('spwrk_post_grid','spwrk_post_grid_shortcode_mapper');

// Load Posts Ajax actions
add_action('wp_ajax_spwrk_filter_posts', 'spwrk_post_grid_load_posts_ajax_functions');
add_action('wp_ajax_nopriv_spwrk_filter_posts', 'spwrk_post_grid_load_posts_ajax_functions');

// Load Posts Ajax function
function spwrk_post_grid_load_posts_ajax_functions(){
	// Verify nonce
  	// if( !isset( $_POST['spwrk_ajax_nonce'] ) || !wp_verify_nonce( $_POST['spwrk_ajax_nonce'], 'spwrk_ajax_nonce' ) )
	...
	if( $query->have_posts() ): ?>
		...
		<div class="<?php echo esc_attr( "spwrk_post_grid am__col-3 am_layout_{$layout} {$has_animation_class} " ); ?>">
			...
			<div class="am_grid_col">
				<div class="am_single_grid">
					...
					<div class="am_cont">
						...
						<a href="<?php echo get_the_permalink(); ?>" class="am__readmore"><?php echo esc_html__('Read More','spwrk_post_grid');?></a>
					</div>
				</div>
			</div>
		...
		<?php
			...
			if( $pagination_type == 'load_more' ){
				...
				if( $paginate_links && $dataPaged < $query->max_num_pages ){
					echo "<button type='button' data-paged='{$dataPaged}' data-next='{$dataNext}' class='{$infinite_scroll_class} am-post-grid-load-more'>".esc_html__( 'Load More', 'spwrk_post_grid' )."</button>";
				}
		...
		esc_html_e('No Posts Found','spwrk_post_grid');
	endif;
	wp_reset_query();
...
function am_ajax_post_grid_plugin_action_links( $links ) {
	$plugin_links = array(
		'<a href="'.admin_url( 'admin.php?page=ajax-post-grid' ).'">' . esc_html__( 'Options', 'spwrk_post_grid' ) . '</a>',
	);
